...
usage() {
    cat <<'USAGE'
Использование:
  bash tools/wb8/run_dev010b_qlcplus_source_switch_acceptance.sh user@wb8-host QLC_PLUS_PC_IP [fixture-start-address]

DEV-010B physical Source switching acceptance:
  - единый runtime MQTT + Art-Net + Source Router + DmxOutput;
  - реальный WB MQTT -> physical DMX;
  - QLC+ Art-Net продолжает обновляться в background при Source=MQTT;
  - MQTT -> ART-NET переключается целым snapshot на physical frame boundary;
  - MQTT logical state продолжает обновляться при Source=ART-NET;
  - ART-NET -> MQTT возвращает latest whole MQTT snapshot;
  - physical DMX остаётся fixed 44 Hz без missed deadlines.

Art-Net Port-Address для этого acceptance = 0.
...
REPORT="${REPO_ROOT}/docs/DEV010B_ARTNET_MQTT_SOURCE_SWITCH_REPORT.txt"
REMOTE_DIR="/tmp/dmxwb-dev010b-source-switch"
CONTROL_DIR="${TMPDIR:-/tmp}/dmxwb-dev010b-ssh-${USER:-user}"
...
cleanup() {
    ...
    if (( SSH_OPENED == 1 )); then
        if (( RUNTIME_STARTED == 1 && TEST_COMPLETE == 0 )) && [[ -n "${RUNTIME_PID}" ]]; then
            # Best-effort safe logical OFF while the unified runtime is still
            # alive. Do not start a competing serial diagnostic.
            ...
            remote "tail -n 160 '${REMOTE_DIR}/runtime.log' 2>/dev/null || true" | tee -a "${REPORT}" >/dev/null || true
        fi
...
{"version":1,"revision":1,"dmx":{"port":"${PORT}"},"artnet":{"universe":${PORT_ADDRESS}},"fixtures":{"count":1,"start_address":${START_ADDRESS},"items":[{"id":1,"name":"DEV010B Source Fixture"}]},"groups":[],"scenes":[],"id_counters":{"next_fixture_id":2,"next_group_id":1,"next_scene_id":1}}
JSON
...
: > "${REPORT}"
record "=== DMXWB DEV-010B QLC+ PHYSICAL SOURCE SWITCHING ==="
...
echo "DMXWB — DEV-010B physical Source switching"
echo "WB8:             ${TARGET}"
echo "QLC+ PC:         ${QLC_IP}"
echo "Fixture RGBW:    channels ${R}/${G}/${B}/${W}"
echo "Art-Net Universe: ${PORT_ADDRESS}"
echo
echo "В тесте физический свет будет переключаться между WB MQTT и QLC+ Art-Net."
echo "На текущем стенде ${PORT} должен быть свободен от WB Serial Device Driver."
echo
...
record "initial_mqtt_snapshot_routed: PASS"

# MQTT owns the physical output first.
...
wait_topic "/devices/dmxwb_fixture_1/controls/brightness" "100" 20 >/dev/null
wait_topic "/devices/dmxwb_fixture_1/controls/color" "255;0;0" 20 >/dev/null
wait_topic "/devices/dmxwb_fixture_1/controls/power" "1" 20 >/dev/null
if ask_yes_no "При Source=WB MQTT светильник физически КРАСНЫЙ?"; then
...
# Art-Net keeps running in background while MQTT remains physically selected.
echo
echo "=== QLC+ background Art-Net ==="
echo "В QLC+ используйте Universe 1 с Art-Net OUTPUT:"
echo "  Network interface:        ${QLC_IP}"
echo "  Destination / IP Address: ${WB_IP}"
echo "  Art-Net Universe:         ${PORT_ADDRESS}"
echo "  Transmission Mode:        Standard"
echo "В Simple Desk выставьте RGBW-каналы светильника:"
echo "  Channel ${R} = 0"
echo "  Channel ${G} = 255"
echo "  Channel ${B} = 0"
echo "  Channel ${W} = 0"
echo "Art-Net OUTPUT должен оставаться включённым."
if ! ask_yes_no "QLC+ сейчас отправляет эти значения на ${WB_IP}?"; then
    record "qlc_green_background_user: FAIL"
    exit 1
fi
wait_runtime_value status_has_artnet_snapshot 1 50 >/dev/null
record "artnet_background_snapshot_received: PASS"
if [[ "$(mqtt_get "/devices/dmxwb/controls/source" || true)" != "mqtt" ]]; then
    echo "Source неожиданно изменился без MQTT-команды." >&2
    exit 1
fi
if ask_yes_no "Несмотря на зелёный Art-Net в фоне, физический свет всё ещё КРАСНЫЙ?"; then
    record "background_artnet_does_not_override_mqtt_user: PASS"
else
    record "background_artnet_does_not_override_mqtt_user: FAIL"
    exit 1
fi

# Explicit MQTT -> ART-NET switch.
echo
echo "=== MQTT -> ART-NET ==="
...
record "mqtt_to_artnet_selected: PASS"
if ask_yes_no "После переключения Source светильник физически стал ЗЕЛЁНЫМ?"; then
    record "mqtt_to_artnet_green_physical_user: PASS"
else
    record "mqtt_to_artnet_green_physical_user: FAIL"
    exit 1
fi

# MQTT logical model must keep changing while Art-Net stays physically selected.
echo
echo "=== Background MQTT while ART-NET selected ==="
mqtt_pub "/devices/dmxwb_fixture_1/controls/brightness/on" "100"
...
wait_topic "/devices/dmxwb_fixture_1/controls/power" "1" 20 >/dev/null
record "background_mqtt_model_blue: PASS"
if ask_yes_no "MQTT state уже синий, но физический свет по-прежнему ЗЕЛЁНЫЙ от Art-Net?"; then
    record "background_mqtt_does_not_override_artnet_user: PASS"
else
    record "background_mqtt_does_not_override_artnet_user: FAIL"
    exit 1
fi

# Explicit ART-NET -> MQTT must restore the latest whole MQTT snapshot.
echo
echo "=== ART-NET -> MQTT ==="
...
record "artnet_to_mqtt_selected: PASS"
if ask_yes_no "После возврата Source на WB MQTT светильник физически стал СИНИМ?"; then
    record "artnet_to_mqtt_latest_blue_physical_user: PASS"
else
    record "artnet_to_mqtt_latest_blue_physical_user: FAIL"
    exit 1
fi

# Safe final state through the selected MQTT source.
mqtt_pub "/devices/dmxwb_fixture_1/controls/power/on" "0"
wait_topic "/devices/dmxwb_fixture_1/controls/power" "0" 20 >/dev/null
if ask_yes_no "Финальный Power OFF полностью выключил светильник?"; then
...
FRAMES_BEFORE_STOP="$(wait_runtime_uint_ge status_dmx_frames_sent 1 30)"
...
MQTT_RX="$(final_value final_router_mqtt_snapshots_received || true)"
DMX_FRAMES="$(final_value final_dmx_frames_sent || true)"
for pair in "SWITCHES:${SWITCHES}" "ARTNET_RX:${ARTNET_RX}" "MQTT_RX:${MQTT_RX}" "DMX_FRAMES:${DMX_FRAMES}"; do
    name="${pair%%:*}"
    value="${pair#*:}"
    if [[ ! "${value}" =~ ^[0-9]+$ ]]; then
...
if (( SWITCHES < 2 )); then
    echo "Ожидалось минимум 2 source switches, получено ${SWITCHES}." >&2
    exit 1
fi
if (( ARTNET_RX < 1 )); then
    echo "Не зафиксирован Art-Net snapshot в Source Router." >&2
    exit 1
fi
if (( MQTT_RX < 3 )); then
    echo "Недостаточно MQTT snapshots для background/return проверки: ${MQTT_RX}." >&2
    exit 1
fi
...
record "router_mqtt_snapshots: PASS (${MQTT_RX})"
...
TEST_COMPLETE=1
record "dev010b_physical_source_switch_result: PASS"
record "=== DMXWB DEV-010B QLC+ PHYSICAL SOURCE SWITCH PASS ==="
echo
echo "=== DMXWB DEV-010B QLC+ PHYSICAL SOURCE SWITCH PASS ==="
echo "Report: ${REPORT}"
