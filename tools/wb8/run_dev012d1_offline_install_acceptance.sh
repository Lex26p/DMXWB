...
if [[ $# -gt 2 ]]; then
    echo "Usage: bash tools/wb8/run_dev012d1_offline_install_acceptance.sh [SSH_TARGET] [FIXTURE_START_ADDRESS]" >&2
    exit 2
fi
...
REPORT="${REPO_ROOT}/docs/DEV012D1_OFFLINE_INSTALL_REPORT.txt"
REMOTE_DIR="/tmp/dmxwb-dev012d1-offline"
...
for command_name in awk cat date grep mkdir mktemp python3 rm scp sed seq sha256sum sleep ssh tar tee tr; do
...
if [[ ! -f "${ARCHIVE}" ]]; then
    ...
    echo "Сначала должен быть получен PASS DEV-012C4." >&2
    exit 1
fi
...
if [[ -z "${SOURCE_ID}" || -z "${ARTIFACT_SHA256}" ]]; then
    echo "Final bundle manifest не содержит обязательную identity." >&2
    exit 1
fi

cat > "${LOCAL_TMP}/config.json" <<JSON
{"version":1,"revision":1,"dmx":{"port":"${DMX_PORT}"},"artnet":{"universe":${ARTNET_PORT_ADDRESS}},"fixtures":{"count":1,"start_address":${START_ADDRESS},"items":[{"id":1,"name":"DEV012D1 Offline Fixture"}]},"groups":[],"scenes":[],"id_counters":{"next_fixture_id":2,"next_group_id":1,"next_scene_id":1}}
JSON
...
wait_service_active() {
    ...
    for _ in $(seq 1 80); do
        if remote "systemctl is-active --quiet dmxwb.service"; then
            pid="$(remote "systemctl show -p MainPID --value dmxwb.service" | tr -d '\r\n')"
            if [[ "${pid}" =~ ^[1-9][0-9]*$ ]]; then
                printf '%s' "${pid}"
                return 0
            fi
...
record "=== DMXWB DEV-012D1 CLEAN OFFLINE INSTALL ACCEPTANCE ==="
...
echo "DMXWB — DEV-012D1 clean offline installation"
echo "WB8:           ${TARGET}"
...
if ! ask_yes_no "Внешний Интернет для WB8 физически недоступен, но локальная LAN/SSH работает?"; then
    exit 2
fi
...
remote "for c in awk bash cat cmp cp find grep install journalctl ldd mkdir mosquitto_pub mosquitto_sub python3 readlink rm sha256sum stat systemctl tar timeout tr uname wc; do command -v \$c >/dev/null || { echo missing:\$c >&2; exit 1; }; done"
...
if [[ -n "${RUNNING_DMXWB}" ]]; then
    echo "На WB8 уже запущен DMXWB. Остановите его перед clean-install acceptance:" >&2
    printf '%s\n' "${RUNNING_DMXWB}" >&2
    exit 1
fi
...
record "clean_install_preflight: PASS"
...
echo
echo "=== Transfer and offline installation ==="
...
remote "test \"\$(sha256sum '${REMOTE_ARCHIVE}' | awk '{print \$1}')\" = '${ARCHIVE_SHA256}'"
record "transferred_bundle_sha256: PASS (${ARCHIVE_SHA256})"

remote "mkdir -p '${REMOTE_DIR}/bundle'; tar -C '${REMOTE_DIR}/bundle' -xzf '${REMOTE_ARCHIVE}'"
remote "test \"\$(find '${REMOTE_BUNDLE}' -type f | wc -l | tr -d ' ')\" = 12"
remote "test ! -d '${REMOTE_BUNDLE}/src'; test ! -d '${REMOTE_BUNDLE}/include'; test ! -d '${REMOTE_BUNDLE}/tests'"
remote "cd '${REMOTE_BUNDLE}' && bash ./install.sh" | tee -a "${REPORT}"
record "offline_bundle_install: PASS"
...
remote "
cmp -s '${REMOTE_BUNDLE}/payload/etc/systemd/system/dmxwb.service' /etc/systemd/system/dmxwb.service
for name in index.html app.js model.js mqtt-client.js styles.css; do
    cmp -s \"${REMOTE_BUNDLE}/payload/var/www/dmxwb/\${name}\" \"/var/www/dmxwb/\${name}\"
done
test \"\$(stat -c '%a' /usr/local/bin/dmxwb)\" = 755
for path in /etc/dmxwb/config.json /etc/systemd/system/dmxwb.service /var/www/dmxwb/index.html /var/www/dmxwb/app.js /var/www/dmxwb/model.js /var/www/dmxwb/mqtt-client.js /var/www/dmxwb/styles.css; do
    test \"\$(stat -c '%a' \"\${path}\")\" = 644
done
for path in /etc/dmxwb /var/lib/dmxwb /var/www/dmxwb; do
...
PY"
record "installed_layout_service_local_mqtt_web: PASS (pid=${PID_INITIAL})"

echo "Откройте http://${TARGET_HOST}/dmxwb/ и обновите страницу через Ctrl+F5."
if ! ask_yes_no "Web-интерфейс DMXWB открылся из локальной LAN и показывает связь с работающей службой?"; then
    exit 1
fi
record "local_lan_web_user: PASS"

echo
echo "=== Basic MQTT to physical DMX ==="
...
PID_FIXTURE="$(wait_service_active)"
wait_topic "/devices/dmxwb/controls/status" running 60
...
wait_topic "/devices/dmxwb_fixture_1/controls/green" 0 60
wait_topic "/devices/dmxwb_fixture_1/controls/blue" 0 60
if ! ask_yes_no "После MQTT-команд физический светильник стабильно КРАСНЫЙ?"; then
    exit 1
fi
record "mqtt_fixture_physical_red_user: PASS (pid=${PID_FIXTURE})"
...
if ! ask_yes_no "Перед восстановлением среды физический светильник полностью выключен?"; then
    exit 1
fi
...
record "dev012d1_result: PASS"
record "=== DMXWB DEV-012D1 CLEAN OFFLINE INSTALL ACCEPTANCE PASS ==="

echo
echo "=== DMXWB DEV-012D1 CLEAN OFFLINE INSTALL ACCEPTANCE PASS ==="
echo "Report: ${REPORT}"
