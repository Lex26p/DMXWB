...
### Планирование нового engineering gate

Перед реализацией **первого** шага каждого нового engineering gate ассистент обязан сначала изучить gate целиком:

1. перечитать обязательные документы в установленном порядке;
2. изучить актуальную реализацию всех подсистем, непосредственно относящихся к gate;
3. сопоставить требования `TECHNICAL_SPEC.md`, scope/PASS из `ROADMAP.md` и фактические пробелы текущего кода;
4. решить, безопасно ли выполнять gate одним шагом.

Если gate достаточно большой для разбиения:

- **до начала реализации** перечислить все планируемые подпункты gate в порядке выполнения;
- для каждого подпункта заранее указать его scope и PASS-критерий;
- зафиксировать этот план в `docs/PROJECT_STATE.md`, если gate займёт несколько handoff/commit;
- не добавлять, не переставлять и не расширять подпункты молча по ходу разработки.

Если новая фактическая информация делает исходный план неверным, ассистент сначала останавливает реализацию, явно объясняет причину и предлагает **явную ревизию всего оставшегося плана**. Только после такой ревизии продолжается разработка.

Разбиение gate на подпункты является execution plan и само по себе не изменяет нормативный scope/PASS gate. `ROADMAP.md` меняется только при реальном изменении порядка, scope или PASS criteria roadmap.
