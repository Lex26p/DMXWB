...
передача 0x00
wait/drain до физического завершения передачи
возврат на 250000 8N2
передача Start Code + data
```

Конкретная реализация C++ должна быть проверена на реальном WB8 и, при наличии безопасного измерительного оборудования, осциллографом/логическим анализатором.

Программный transport не должен изменяться на kernel/WBEC-реализацию без отдельного пересмотра ТЗ.
