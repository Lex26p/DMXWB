...
DMXWB — специализированная программная подсистема контроллера Wiren Board, реализованная как один C++20 daemon и предназначенная для непрерывного управления одной физической линией DMX512 через встроенный RS-485 контроллера.

DMXWB не рассматривается как самостоятельная универсальная платформа или отдельный сервер автоматизации. Приложение является частью программной среды Wiren Board и интегрируется с локальными сервисами контроллера.
...
- DMXWB является компонентом Wiren Board и использует локальную инфраструктуру контроллера, а не создаёт отдельную универсальную платформу;
- работа DMXWB не должна зависеть от доступности внешнего интернета.
...
Эксплуатационная модель DMXWB предполагает доверенную локальную LAN. В рамках DMXWB не проектируются и не настраиваются собственные механизмы authentication/authorization, Mosquitto ACL, политика anonymous access или отдельная модель контроля доступа к `/mqtt`. Эти вопросы относятся к общей сетевой и системной конфигурации Wiren Board и находятся вне scope настоящего ТЗ. Собственный web DMXWB не содержит отдельной системы пользователей или входа в приложение.
...
### 22.1. Офлайн-установка на Wiren Board

Установка конечного DMXWB на поддерживаемый Wiren Board должна выполняться полностью офлайн, без доступа в интернет.

Финальный installation bundle должен содержать готовый production binary `dmxwb`, статические web-файлы, systemd unit, необходимые deployment-файлы и все прочие файлы, требуемые для установки приложения.

Production installer:

- не должен выполнять `apt update` или скачивать пакеты из сетевых репозиториев;
- не должен выполнять `git clone`, `curl`, `wget` или иные загрузки из интернета;
- не должен требовать `npm`, Node.js или frontend build step;
- не должен требовать C++ compiler или CMake на целевом Wiren Board;
- может использовать только компоненты, гарантированно присутствующие в поддерживаемой базовой системе Wiren Board;
- если обязательная runtime-зависимость не гарантирована базовой системой, совместимая зависимость должна поставляться локально в составе installation bundle.

Среда разработки и сборки может использовать необходимые инструменты отдельно от целевого контроллера. Это не отменяет требования полностью офлайн-установки production bundle на Wiren Board.

После установки и перезагрузки DMXWB должен запускаться и выполнять все основные функции без внешнего интернет-доступа. Для эксплуатации требуется только локальная инфраструктура контроллера и локальная сеть, необходимая для MQTT/Web/Art-Net.
...
### 23.4. Offline installation

На поддерживаемом Wiren Board с физически недоступным внешним интернетом проверяется финальный installation bundle. Локальная LAN при этом может использоваться для доступа к web и Art-Net.

Проверяется:

- установка только из локального installation bundle;
- отсутствие сетевых загрузок и online package installation;
- отсутствие необходимости C++/CMake/Node.js toolchain на целевом WB;
- успешный запуск systemd service после установки и reboot;
- загрузка собственного web из локальной LAN;
- подключение backend к локальному Mosquitto;
- базовый физический DMX output;
- приём Art-Net из локальной сети.

PASS: установка, запуск и базовая работа DMXWB не требуют внешнего интернет-доступа.
...
24. **DMXWB является специализированной подсистемой Wiren Board, а не отдельной универсальной платформой.**
25. **Установка и работа DMXWB не зависят от внешнего интернет-доступа.**
26. **DMXWB рассчитан на доверенную локальную LAN; authentication/authorization, Mosquitto ACL и политика доступа к `/mqtt` находятся вне scope приложения.**
...
Конечный DMXWB должен представлять собой специализированный C++20 daemon, интегрированный как программная подсистема Wiren Board, с:
...
fully offline installation on supported Wiren Board
```

Главный конечный критерий — после полностью офлайн-установки на поддерживаемый Wiren Board система должна стабильно управлять реальным освещением через физический DMX512 и принимать внешнее управление по Art-Net, сохраняя локальное управление и интеграцию с Wiren Board.
